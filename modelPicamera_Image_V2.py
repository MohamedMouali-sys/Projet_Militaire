...
score1, detections1 = get_model_score(model1, image, class_names_model1)
score2, detections2 = get_model_score(model2, image, class_names_model2)
...
if best_detections:
    for box, class_id, conf, class_name in best_detections:
        ...
        cvzone.putTextRect(image, f"{class_name}: {conf:.2f}", (x1, max(y1 - 10, 10)), scale=1, thickness=1)
else:
